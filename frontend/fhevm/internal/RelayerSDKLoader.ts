    ...
    return new Promise((resolve, reject) => {
      ...
      const tryLoad = (src: string, onError?: () => void) => {
        ...
        script.onerror = () => {
          ...
          reject(
            new Error(
              `RelayerSDKLoader: Failed to load Relayer SDK from ${src}`
            )
          );
        };
...
function isFhevmRelayerSDKType(
  ...
  // Accept either ZamaEthereumConfig (v0.9) or SepoliaConfig (older bundles)
  const hasZamaEthereumConfig = objHasProperty(o, "ZamaEthereumConfig", "object", trace);
  const hasSepoliaConfig = objHasProperty(o as object, "SepoliaConfig" as never, "object", trace);
  if (!hasZamaEthereumConfig && !hasSepoliaConfig) {
    trace?.("RelayerSDKLoader: relayerSDK config object is missing (ZamaEthereumConfig/SepoliaConfig).");
    return false;
  }
