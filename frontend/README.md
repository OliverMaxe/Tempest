# Tempest Sentinel Frontend

基于 Next.js + Zama FHEVM 的 Tempest Sentinel 前端，配合 `../contracts` 下的 `TempestSentinel.sol` 合约运行。支持：

- 本地 Hardhat FHEVM 节点：自动切换到 MockFhevmInstance。
- Sepolia 测试网：通过 `@zama-fhe/relayer-sdk` 与链上合约交互。
...
> 本模板已内置合约与前端，无需额外 clone 子仓库。按照下方步骤分别安装依赖即可。
...
3. Deploy `TempestSentinel` to the local node:

```sh
# still in action/contracts
...
- **`action/frontend/hooks/useTempestSentinel.tsx`**：展示如何在 React 中集成 `useFhevm` 与 TempestSentinel 合约。
